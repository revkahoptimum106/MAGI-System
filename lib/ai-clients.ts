...
function parseVoteResponse(text: string): { reasoning: string; vote: Vote } {
  const cleaned = text.trim().replace(/^```json\s*/i, "").replace(/```\s*$/i, "").trim();
  const parsed = JSON.parse(cleaned);
  const vote = (["APPROVE", "REJECT", "ABSTAIN"].includes(parsed.vote)
    ? parsed.vote
    : "ABSTAIN") as Vote;
  return { reasoning: parsed.reasoning || "", vote };
}
...
export async function queryCasper(topic: string): Promise<{ reasoning: string; vote: Vote }> {
